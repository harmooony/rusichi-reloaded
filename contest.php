...
<body>
      ...
      <div class="contest_container">
        <div>
          ...
          <div class="contest_member">
            ...
            <div style="border-radius: 20px;" class="contest_graph">
            ...
            <div class="contest_indrate">
              ...
              <input type="text" class="admin_input" id="search1">
              <table id="table1" style="width:100%; padding-bottom: 20px; margin-top: 20px">
                <tr>
                ...
                foreach ($row as $item) {
                  ...
                  <tr>
                    ...
                    <?php $sum1 = $sum1 + $item[2] ?>
                  </tr>

                  <?php

                }
                $jsonSum1 = json_encode($sum1); ?>
        ...
        <div>
          ...
          <div class="contest_member">
            ...
            <div class="contest_indrate">
              <p class="contest_indrate_text">Индивидуальный рейтинг: </p>
              <input type="text" class="admin_input" id="search2">
              <table id="table2" style="width:100%; padding-bottom: 20px; margin-top: 20px">
                <tr>
                  <th class="rate_name" style="width: 50%;">Имя</th>
                ...
                foreach ($row as $item) {
                  ...
                  <tr>
                    <th style="text-align: center; font-size: 20px; width: 50%;"><?= $item[0] . " " . $item[1] ?></th>
                    <th style="text-align: center; font-size: 20px; width: 50%;"><?= $item[2] ?></th>
                    <?php $sum2 = $sum2 + $item[2] ?>
                  </tr>

                  <?php

                }
                $jsonSum2 = json_encode($sum2); ?>
  ...
  <script>
    var sum1 = <?php echo $jsonSum1; ?>;
    var sum2 = <?php echo $jsonSum2; ?>;
    document.getElementById('grup1').innerHTML = "Общий рейтинг команды: " + (total1 + sum1 * 0.1);
    document.getElementById('grup2').innerHTML = "Общий рейтинг команды: " + (total2 + sum2 * 0.1);
    var sum_members;
    function update(id, column) {
      ...
      if (column == 1) {
        tekChart = 'chart1'
        tekTable = '#table1'
        localStorage.setItem('left', id);
      } else if (column == 2) {
        tekChart = 'chart2'
        tekTable = '#table2'
        localStorage.setItem('right', id);
      }
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          $.each(response, function (i, item) {
            ...
            sum_members += parseInt(item.Инд_рейтинг);
          });
    ...
    function test1() {
      var chartData1 = 0;
      for (let i = 0; i < myChart1.data.datasets[0].data.length; i++) {
        chartData1 += parseInt(myChart1.data.datasets[0].data[i]);
      }
      document.getElementById('grup1').innerHTML = "Общий рейтинг команды: " + (chartData1 + sum_members * 0.1);

    }

    function test2() {
      var chartData2 = 0;
      for (let i = 0; i < myChart2.data.datasets[0].data.length; i++) {
        chartData2 += parseInt(myChart2.data.datasets[0].data[i]);
      }
      document.getElementById('grup2').innerHTML = "Общий рейтинг команды: " + (chartData2 + sum_members * 0.1);
    }
